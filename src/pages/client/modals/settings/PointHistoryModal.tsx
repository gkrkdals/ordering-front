...
import {dateToString, formatDate} from "@src/utils/date.ts";
import {
...
/** 'M월 D일 ~ M월 D일' */
function formatPeriod(dates: Date[]) {
  const label = (date: Date) => `${date.getMonth() + 1}월 ${date.getDate()}일`;
  return `${label(dates[0])} ~ ${label(dates[1])}`;
}
...
interface PointHistoryModalProps extends BasicModalProps {
  /** 달력에서 고른 조회 기간 (시작일, 종료일) */
  selectedDates: Date[];
  setSelectedDates: (dates: Date[]) => void;
}

export default function PointHistoryModal({ selectedDates, setSelectedDates, ...props }: PointHistoryModalProps) {
  ...
  useEffect(() => {
    if (!props.open || selectedDates.length !== 2) {
      return;
    }
    ...
    client
      .get('/api/order/point/history', {
        params: {
          startDate: dateToString(selectedDates[0]).split(' ').at(0),
          endDate: dateToString(selectedDates[1]).split(' ').at(0),
        },
      })
      .then(res => {
    ...
    <Dialog open={props.open} fullWidth maxWidth='sm'>
      ...
      <DialogContent>
        ...
        {selectedDates.length === 2 && (
          <p className='text-secondary text-center pt-2 mb-0' style={{ fontSize: FONT.sub }}>
        ...
        {!loading && !failed && histories.length >= HISTORY_LIMIT && (
          <p className='text-secondary text-center pt-3 mb-0' style={{ fontSize: FONT.sub }}>
            {HISTORY_LIMIT}건까지만 표시됩니다. 기간을 좁혀 조회해 주세요.
          </p>
        )}
      </DialogContent>
      <DialogActions>
        <SecondaryButton style={{ fontSize: FONT.button }} onClick={handleClose}>
          닫기
        </SecondaryButton>
      </DialogActions>
    </Dialog>
